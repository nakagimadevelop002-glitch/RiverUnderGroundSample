...
namespace KWS
{
    internal static class KW_Extensions
    {
...
#if UNITY_EDITOR
            return (Application.isPlaying ? Time.time  : Time.realtimeSinceStartup) * WaterSystem.GlobalTimeScale;
#else
        ...
        static string GetPathToKriptoWaterSystemFolder()
        {
            ...
            if (dirs.Length != 0)
            {
                var pathToShadersFolder = Path.Combine(dirs[0], "WaterSystem2");
        ...
        public static string GetFullPathToResourcesFolder()
        {
            ...
            var pathToSavedDataFolder = Path.Combine(waterFolder, "WaterQualitySettings", "Resources");
        ...
        public static string GetRelativeToAssetsPath(this string path)
        {
            path = Path.Combine("Assets", Path.GetRelativePath("Assets", path));
            path = path.Replace("\\", "/");
            return path;
